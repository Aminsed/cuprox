...
class TestQPLayerForward:
    ...
    def test_unconstrained_qp(self, device, dtype):
        ...
        torch.testing.assert_close(
            x, torch.tensor([1., 2.], device=device, dtype=dtype), atol=1e-3, rtol=1e-3
        )
    ...
    def test_box_constrained_qp(self, device, dtype):
        ...
        torch.testing.assert_close(
            x, torch.tensor([1., 1.], device=device, dtype=dtype), atol=1e-3, rtol=1e-3
        )
    ...
    def test_equality_constrained_qp(self, device, dtype):
        ...
        torch.testing.assert_close(
            x, torch.tensor([1., 1.], device=device, dtype=dtype), atol=1e-2, rtol=1e-2
        )
    ...
    def test_single_variable(self, device, dtype):
        ...
        torch.testing.assert_close(
            x, torch.tensor([2.], device=device, dtype=dtype), atol=1e-3, rtol=1e-3
        )
