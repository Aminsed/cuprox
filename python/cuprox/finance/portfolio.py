...
class Portfolio:
    ...
    def optimize(
        ...
        if method == "mean_variance":
            ...
            return self._optimize_target_volatility(target_volatility, max_iters, tolerance, verbose)
        else:
            raise ValueError(
    ...
    def _optimize_target_volatility(
        ...
        for _ in range(50):
            ...
            result = self._optimize_mean_variance(
                lambda_mid, max_iters, tolerance, verbose=False
            )
    ...
    def _make_result(
        ...
        if hasattr(solve_result, 'status'):
            status = str(solve_result.status.value) if hasattr(solve_result.status, 'value') else str(solve_result.status)
        else:
            status = "optimal"
