...
def solve_cuprox(c, A, b, lb, ub, **kwargs):
    ...
    result = cuprox.solve(
        ...
        ub=ub,
        params={
            "max_iterations": kwargs.get("max_iters", 10000),
...
def benchmark_single(n, m, density=0.1, seed=42):
    ...
    results["scipy"] = res
    print(
        f"    SciPy:   {res['time']*1000:8.1f} ms, obj={res['objective']:10.4f}, "
        f"status={res['status']}"
    )
...
def benchmark_batch():
    ...
    for batch_size in batch_sizes:
        ...
        print(
            f"  SciPy (sequential): {scipy_time*1000:.1f} ms total, "
        ...
        if HAS_CUPROX and cuprox.__cuda_available__:
            ...
            print(
                f"  cuProx (sequential): {cuprox_time*1000:.1f} ms total, "
