...
# Deliberately no sys.path manipulation. Pointing at python/ imports the source
# tree, which contains no compiled _core, so the benchmark would silently time
# the scipy fallback while printing a GPU header. Install the package first:
#   pip install .
...
try:
    ...
    if cuprox.__cuda_available__:
        ...
        print(
            "WARNING: cuProx is installed without a working CUDA extension.\n"
            "         These numbers would describe the scipy fallback, not the GPU.\n"
            "         Build with `pip install .` from the repository root."
        )
        sys.exit(1)
except ImportError:
    HAS_CUPROX = False
...
def solve_cuprox_qp(P, q, A, l, u, **kwargs):
    ...
    result = cuprox.solve(
        ...
        constraint_l=l,
        constraint_u=u,
        # Match OSQP: variables are free unless a row of A says otherwise.
        # cuprox.solve defaults lb to zeros (LP standard form), so omitting
        # these compares x >= 0 against an unconstrained problem -- which is
        # how this benchmark reported wildly different objectives for the two
        # solvers while both said "optimal".
        lb=np.full(n, -np.inf),
        ub=np.full(n, np.inf),
        params={
            "max_iterations": kwargs.get("max_iters", 4000),
...
def benchmark_single(n, m, density=0.1, seed=42):
    ...
    if HAS_OSQP:
        ...
        results["osqp"] = res
        print(
            f"    OSQP:    {res['time']*1000:8.1f} ms, obj={res['objective']:10.4f}, "
    ...
    if HAS_CUPROX and cuprox.__cuda_available__:
        ...
        results["cuprox"] = res
        print(
            f"    cuProx:  {res['time']*1000:8.1f} ms, obj={res['objective']:10.4f}, "
            f"iters={res['iterations']}, status={res['status']}"
        )
